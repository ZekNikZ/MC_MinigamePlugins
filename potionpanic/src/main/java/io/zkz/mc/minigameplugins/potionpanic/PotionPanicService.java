...
public class PotionPanicService extends PluginService<PotionPanicPlugin> {
...
//            TitleUtils.broadcastTitle(ChatColor.RED + "Round starts in 20 seconds", ChatColor.GOLD + "Find a good starting position!", 10, 70, 20);
    ...
    protected void onEnable() {
        MinigameService minigame = MinigameService.getInstance();
        minigame.registerRounds(minigame.getPlayers().stream().sorted(Comparator.comparing(p -> p)).map(PotionPanicRound::new).toArray(PotionPanicRound[]::new));
    }
